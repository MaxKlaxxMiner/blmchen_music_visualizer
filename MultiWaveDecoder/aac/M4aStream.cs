...
{
  public sealed class M4AStream
  {
    readonly byte[] data;
    int dataPos;

    public M4AStream(byte[] data)
    {
      this.data = data;
      dataPos = 0;
    }

    public int Offset { get { return dataPos; } }
    ...
    public string PeekString(int offset, int length, Encoding encoding = null)
    {
      ...
      return encoding.GetString(data, dataPos + offset, length);
    }
    ...
    public string ReadString(int length, Encoding encoding = null)
    {
      ...
      return encoding.GetString(data, dataPos += length, length);
    }

    long Read(int bytes, bool littleEndian = false)
    {
      if (littleEndian) throw new NotSupportedException();

      long result = 0;
      while (--bytes >= 0)
      {
        result += (long)data[dataPos++] << bytes * 8;
      }
      return result;
    }
    ...
    {
      return (uint)Read(4, littleEndian);
    }
  }
}
