...
{
  /// <summary>
  /// The <code>DecoderConfigDescriptor</code> provides information about the decoder type and the required decoder resources needed for the associates
  /// elementary stream. This is needed at the receiver to determine whether it is able to decode the elementary stream. A stream type identifies the category
  /// of the stream while the optional decoder specific information contains stream specific information for the set up of the decoder in a stream specific format that is opaque to this layer.
  /// </summary>
  public sealed class DecoderConfigDescriptor : Descriptor
  {
    int objectProfile, streamType, decodingBufferSize;
    bool upstream;
    long maxBitRate, averageBitRate;
    ...
    /// <summary>
    /// An indication for the object profile (or scene description profile if streamType==scene description stream), that needs to be supported by the
    /// decoder for this elementary stream as per the following table. For streamType==object descriptor stream the value 0xFF ('no profile specified') shall be used.
    /// 
    /// <table>
    ...
    /// </table>
    /// </summary>
    /// <returns>the object profile</returns>
    ...
    /// </summary>
    /// <returns>true if this stream is used for upstream information</returns>
    ...
    /// </summary>
    /// <returns>the decoding buffer size</returns>
    ...
    /// </summary>
    /// <returns>the maximum bitrate</returns>
    ...
    /// <summary>
    /// The average bitrate of the elementary stream. For streams with variable bitrates this value shall be set to zero.
    /// </summary>
    /// <returns>the average bitrate or 0 if the stream has a variable bitrate</returns>
