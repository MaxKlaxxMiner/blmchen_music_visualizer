...
#define PRINT_MP4_METADATA
  ...
  for (sampleId = 0; sampleId < numSamples; sampleId++)
  {
    ...
    if (first_time && !frameInfo.error)
    {
      ...
      {
        /* open output file */
        aufile = open_audio_file(sndfile, frameInfo.samplerate, frameInfo.channels, outputFormat, fileType, aacChannelConfig2wavexChannelMask(&frameInfo));
        if (aufile == NULL)
        {
          NeAACDecClose(hDecoder);
          mp4ff_close(infile);
...
int main(int argc, char *argv[])
{
  ...
  while (1)
  {
    ...
    c = getopt_long(argc, argv, "o:a:s:f:b:l:wgdhitq", long_options, &option_index);
